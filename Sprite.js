...
(function() {
    ...
    SpriteEvent.prototype.apply = function(me, r) {
        for(var i = 0; i < this.events.length; i++) {
            this.events[i].apply(me, r);
        }
    };
...
(function() {
    ...
    (Sprite.DEFAULTS = function(f) {
        ...
        (function(model, constructor) {        
            if(model.image) {
                if(!(model.image instanceof Array)) {
                    model.image = [model.image];
                }                           
        		var me = this, loaded = 0, image = me.image = model.image, func = function(i) {
            		Utility.img(image[i], function(img) {
            			image[i] = img;
            			model.width = me.width = me.width || model.width || img.width / me.columns;
            			model.height = me.height = me.height || model.height || img.height / me.rows;     
                        loaded++
            ...
            function init() {
            	...
            	if(constructor = (Sprite.FUNCTIONS[constructor] || constructor)) {
    ...
    Sprite.prototype.draw = function(ctx) {
    	if(this.visible) {
    		...
    		if(image && image.complete) {
    			var sw = (image[0].width / this.columns), sh = (image[0].height / this.rows);
            ...
            if(!this.disabled) {
        		...
        		if(ctx.isPointInPath(l[0], l[1])) {
        			m = Matrix.inverse(ctx.getTransform())
        			l = Matrix.multiply(m, mouse.location);	
    ...
    Sprite.prototype.getTransformedPoints = function() {
    	var m = this.matrix;
    	m = Matrix.scale(m, this.sx, this.sy);
    	m = Matrix.translate(m, this.width / 2 + this.location[0] / this.sx, this.height / 2 + this.location[1] / this.sy);
    	m = Matrix.rotate(m, this.r);		
    	var w2 = this.width / 2, h2 = this.height / 2;
    	return this.points = [
    		Matrix.multiply(m, [-w2, -h2, 1]),
    		Matrix.multiply(m, [w2, -h2, 1]),
    		Matrix.multiply(m, [w2, h2, 1]),
    		Matrix.multiply(m, [-w2, h2, 1]),
    	];
    			...
    			if((s1.movement == "d" || s2.movement == "d") && (c1 || c2) && (s1.visible && !s1.disabled && s2.visible && !s2.disabled)) {                                 
    				...
    				for(var k = 0; !intersects && k < p1.length; k++) {
    					for(var l = 0; !intersects && l < p2.length; l++) {
    						intersects = Utility.getLineLineIntersection(p1[k], p1[(k + 1) % p1.length], p2[l], p2[(l + 1) % p2.length]) !== null;
    					}
    				}
    				if(intersects) {
    					if(c1) {
    ...
    Sprite.prototype.drop = function(s, b) {
    	var d = this.ondrop[s.type] || this.ondrop["ALL"];
        if(d) {
    ...
    Sprite.prototype.take = function() {
    	...
    	if(this.droppable && this.droppable.holding.remove(this)) {
            var func;
    		if(this.droppable.ontake && (func = this.droppable.ontake[this.type] || this.droppable.ontake["ALL"])) {
    ...
    Sprite.prototype.find = function(model, r) {
    	r = r || [];
    	var s = this.sprites, f = typeof(model) === "function" ? model : Utility.object_is_model;
